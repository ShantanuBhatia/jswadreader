...
const readREJECT = async (fd, REJECT_entry, SECTORS_entry) => {
	console.log("Reject might come later oops");
	return {
		"data": "no data in REJECT table"
	}
...
const readBLOCKMAP = async (fd, BLOCKMAP_entry) => {
	console.log("Blockmap might come later oops");
	return {
		"data": "no data in BLOCKMAP"
	}

}
/*
	Fun fact: use regex to make finding all the functions in your code easier and then you can autogen the exports object using regex
	I found all the functions in this file by matching for the patterm "^const (\w+) \s*=\s*[async]*\s*\("
	The function name is in the capture field so you can use ` "$1": $1, ` to replace with just the function names. 
*/


module.exports = {
	"itemSizes": itemSizes,
	"getWadHeader":getWadHeader,
	"readDirEntry":readDirEntry,
	"readWadDirectory":readWadDirectory,
	"getAllMaps":getAllMaps,
	"readTHINGS":readTHINGS,
	"readLINEDEFS":readLINEDEFS,
	"readSIDEDEFS":readSIDEDEFS,
	"readVERTEXES":readVERTEXES,
	"readSEGS":readSEGS,
	"readSSECTORS":readSSECTORS,
	"readNODES":readNODES,
	"readSECTORS":readSECTORS,
	"readREJECT":readREJECT,
